...
class Gallery(commands.Cog):
    ...
    async def gallery_random(self, interaction: Interaction):
        ...
        if not safe:
            if self.entries:
                msg = "No safe gallery entries available yet."
                decision = "blocked:nsfw"
            else:
                msg = "No gallery entries available yet."
                decision = "blocked:empty"
            await interaction.response.send_message(msg, ephemeral=True)
            self._log("random", interaction, decision=decision, entry=None)
            return
    ...
    @app_commands.command(name="gallery_reload", description="Reload gallery entries from disk.")
